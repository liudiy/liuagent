学习点 1：沙盒隔离 (Sandbox)
- Deer Flow 怎么做 ：用 SandboxMiddleware 拦截请求，启动 Docker。
- TongAgent 可以怎么做 ：创建一个专用的工具 run_code_in_sandbox 。当 LLM 需要跑代码时，调用这个工具，工具内部去连接 Docker 并返回结果。 不需要改变整体架构。 
学习点 2：澄清机制 (Clarification)
- Deer Flow 怎么做 ：用 ClarificationMiddleware 拦截，阻断 LLM。
- TongAgent 可以怎么做 ：给你的 LLM 加一个系统提示词，告诉它："如果需求不明确，请输出 {"intent": "ask_user", "question": "..."} "。然后在你的 route_intent 边里加个分支，直接导向 generate_response 。 极其优雅！ 
学习点 3：任务拆解 (Sub-agents)
- Deer Flow 怎么做 ：通过复杂的 Prompt 强制模型输出多个并行的 task 工具调用。
- TongAgent 可以怎么做 ：你可以增加一个 Planner Node 。在执行复杂任务前，先让 Planner 生成一个步骤列表 (Plan)，存在 State 的 todos 字段里，然后让 LLM 一步一步去执行。
